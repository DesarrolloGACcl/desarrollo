...
class ExportAccounting(models.TransientModel):
    ...
    def action_export(self):
        ...
        headers = ['Fecha', 'Tipo', 'Documento', 'Proyecto', 'Área', 'Actividad', 'Tarea', 
                   ...
                   'Cuenta', 'Raíz de cuenta', 'Importe en moneda', 'Importe residual',
        ...
        for line in account_move_lines:
            ...
            worksheet.write(row, 12, line.account_id)
            worksheet.write(row, 13, line.account_root_id)
            ...
            worksheet.write(row, 17, line.analytic_line_ids)
            worksheet.write(row, 18, line.asset_ids)
            worksheet.write(row, 19, line.balance)
            worksheet.write(row, 20, line.company_currency_id)
            worksheet.write(row, 21, line.currency_id)
            ...
            worksheet.write(row, 30, line.followup_line_id)
            worksheet.write(row, 31, line.full_reconcile_id)
            worksheet.write(row, 32, line.group_tax_id)
            worksheet.write(row, 33, line.is_downpayment)
            worksheet.write(row, 34, line.journal_id)
            worksheet.write(row, 35, line.l10n_latam_document_type_id)
            worksheet.write(row, 36, line.matching_number)
            worksheet.write(row, 37, line.move_name)
            worksheet.write(row, 38, line.parent_state)
            worksheet.write(row, 39, line.partner_id)
            worksheet.write(row, 40, line.payment_id)
            ...
            worksheet.write(row, 45, line.product_id)
            worksheet.write(row, 46, line.product_uom_id)
            worksheet.write(row, 47, line.purchase_line_id)
            worksheet.write(row, 48, line.quantity)
            worksheet.write(row, 49, line.reconcile_model_id)
            worksheet.write(row, 50, line.reconciled)
            worksheet.write(row, 51, line.ref)
            worksheet.write(row, 52, line.sequence)
            worksheet.write(row, 53, line.statement_id)
            worksheet.write(row, 54, line.statement_line_id)
            worksheet.write(row, 55, line.tax_audit)
            worksheet.write(row, 56, line.tax_base_amount)
            worksheet.write(row, 57, line.tax_group_id)
            worksheet.write(row, 58, line.tax_ids)
            worksheet.write(row, 59, line.tax_line_id)
            worksheet.write(row, 60, line.tax_repartition_line_id)
            worksheet.write(row, 61, line.tax_tag_ids)
