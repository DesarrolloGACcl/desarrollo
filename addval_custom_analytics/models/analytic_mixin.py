...
class AnalyticMixin(models.AbstractModel):
    ...
    analytic_distribution_task_search = fields.Json(
        store=False,
        search='_search_analytic_distribution_task'
    )
    ...
    def init(self):
        ...
        if self.env.cr.dictfetchone(): 
            query = f"""
                CREATE INDEX IF NOT EXISTS {self._table}_analytic_distribution_task_gin_index
                                       ON {self._table} USING gin(analytic_distribution_task);
            """
            self.env.cr.execute(query)
        super().init()
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        if res.get('analytic_distribution_task_search'):
            res['analytic_distribution_activity_search']['searchable'] = False
        return res
    ...
    def _search_analytic_distribution_task(self, operator, value):
        if operator not in ['=', '!=', 'ilike', 'not ilike'] or not isinstance(value, (str, bool)):
            raise UserError(_('Operator nor supported'))
        operator_name_search = '=' if operator in ('=', '!=') else 'ilike'
        account_ids = list(self.env['account.analytic.account'].name_search(name=value, operator=operator_name_search))
    ...
    def _validate_distribution(self, **kwargs):
        if self.env.context.get('validate_analytic', False):
            ...
            for field in ['analytic_distribution', 'analytic_distribution_area', 'analytic_distribution_activity', 'analytic_distribution_task']:
    ...
    def _apply_analytic_distribution_domain(self, domain):
        return[
            ...
            ('analytic_distribution_task_search', leaf[1], leaf[2]) if len(leaf) == 3 and leaf[0] == 'analytic_distribution_task' and isinstance(leaf[2], str) else leaf
            for leaf in domain
        ]